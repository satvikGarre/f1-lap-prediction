...
def get_lap_features(year, gp, session_type, driver_code):
    ...
    features = ['LapTime', 'Sector1Time', 'Sector2Time', 'Sector3Time', 'Compound']
    laps = laps[features]
    # Drop rows with all NaNs in LapTime and sectors (we need these)
...
print_metrics(y_test_inv, y_pred_rf, "Random Forest Baseline")

plt.figure(figsize=(12,6))
plt.plot(y_test_inv, label='Actual Lap Time (s)')
plt.plot(y_pred, label='LSTM Predicted Lap Time (s)')
plt.plot(y_pred_rf, label='Random Forest Predicted Lap Time (s)', linestyle='--')
plt.title('Lap Time Prediction - LSTM vs Random Forest')
